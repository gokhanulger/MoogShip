...
async function recalculateShipment503Pricing() {
  ...
  try {
    // Get user multiplier for user ID 2
    const pool = new Pool({
      connectionString: process.env.DATABASE_URL
    });
    
    const userResult = await pool.query('SELECT multiplier FROM users WHERE id = 2');
    const userMultiplier = userResult.rows[0]?.multiplier || 1.12;
    console.log(`👤 User multiplier: ${userMultiplier}x`);
