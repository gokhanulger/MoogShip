...
async function createAFSWaybill() {
  ...
  try {
    ...
    console.log("URL:", AFS_API_URL);
    console.log("Payload size:", formData.toString().length, "characters");
    ...
    if (!waybillData.hata && waybillData.barkod) {
      ...
      console.log("- Message:", waybillData.mesaj);
      ...
      console.log("Full response:", waybillData);
      return null;
    }
  } catch (error) {
...
async function runCompleteTest() {
  ...
  console.log("\n📦 Complete AFS Transport Payload:");
  console.log(JSON.stringify(waybillPayload, null, 2));
  ...
  if (trackingNumber) {
    // Step 2: Create label
    const labelUrl = await createAFSLabel(trackingNumber);
    
    // Step 3: Get tracking info
    const trackingData = await getAFSTracking(trackingNumber);
    
    console.log("\n📊 Integration Results Summary:");
    console.log("- Tracking Number:", trackingNumber);
    console.log("- Label URL:", labelUrl || "N/A");
    console.log("- Tracking Status:", trackingData?.durum || "N/A");
    ...
    if (labelUrl) {
      console.log("\n🔄 Label PDF Storage:");
      console.log("1. Download PDF from:", labelUrl);
      console.log("2. Store in carrier_label_pdf field");
      console.log("3. Update carrier_label_url field");
    }
  } else {
    console.log("\n❌ Integration test failed - no tracking number generated");
  }
