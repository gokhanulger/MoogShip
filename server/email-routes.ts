...
/* DISABLED - OLD duplicate route
if (false) { // This entire block is disabled and will be removed
  try {
    const passportSession = req.session as any;
    const userId = passportSession.passport.user;
    const connectionId = parseInt(req.params.connectionId);
    ...
    try {
      ...
      console.log('[EMAIL] Searching for Etsy emails since:', twoWeeksAgo.toISOString());
      
      // Search for Etsy emails from the last 2 weeks
      ...
      for (const seq of searchResults) {
        try {
          ...
          if (message && message.source) {
            // Parse email using simpleParser
            const parsed = await simpleParser(message.source);
            
            // Parse the Etsy order from the email
            const orderData = parseEtsyOrderEmail(parsed);
            
            if (orderData) {
              // Check if order already exists
              const existingOrder = await db.query.etsyOrders.findFirst({
                where: and(
                  eq(etsyOrders.userId, userId),
                  eq(etsyOrders.orderNumber, orderData.orderNumber)
                )
              });
              
              if (!existingOrder) {
                // Insert new order
                const [newOrder] = await db.insert(etsyOrders).values({
                  userId,
                  etsyConnectionId: 1, // Default Etsy connection
                  orderNumber: orderData.orderNumber,
                  receiptId: orderData.orderNumber, // Use same value for receiptId
                  buyerName: orderData.buyerName || '',
                  buyerEmail: orderData.buyerEmail || '',
                  shipToName: orderData.shipToName || '',
                  shipToAddress1: orderData.shipToAddress1 || '',
                  ...
                  shipToZip: orderData.shipToZip || '',
                  shipToCountry: orderData.shipToCountry || 'USA',
                  orderDate: parsed.date || new Date(),
                  grandTotal: Math.round((orderData.orderTotal || 0) * 100), // Convert to cents safely
                  subtotal: Math.round((orderData.subtotal || 0) * 100),
                  shippingCost: Math.round((orderData.shippingCost || 0) * 100),
                  taxTotal: Math.round((orderData.taxTotal || 0) * 100),
                ...
                importedCount++;
                console.log(`[EMAIL] Imported new order: ${orderData.orderNumber}`);
              } else {
                console.log(`[EMAIL] Order already exists: ${orderData.orderNumber}`);
              }
            }
          }
        } catch (error) {
          console.error(`[EMAIL] Error processing email ${seq}:`, error);
          errorCount++;
        }
      }
      ...
      await db.update(emailConnections)
        .set({
          lastSyncAt: new Date(),
          totalOrdersImported: db.sql`${emailConnections.totalOrdersImported} + ${importedCount}`,
          connectionStatus: 'connected',
          updatedAt: new Date()
        })
        .where(eq(emailConnections.id, connectionId));
      ...
      console.error('[EMAIL] IMAP error:', imapError);
      ...
      await db.update(emailConnections)
        .set({
          connectionStatus: 'error',
          lastSyncError: imapError.message,
          updatedAt: new Date()
        })
        .where(eq(emailConnections.id, connectionId));
...
} // End of disabled if block
End of removed duplicate route */
