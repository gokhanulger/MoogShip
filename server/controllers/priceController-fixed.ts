    ...
    if (destinationCountry === 'US' && data.hsCode && data.customsValue) {
      try {
        ...
        if (usitcResult.dutyRate && usitcResult.dutyRate.code) {
          ...
          // Check if ECO shipping based on selectedService or shippingMethod
          const isEcoShipping = (data.selectedService && 
            (data.selectedService.toLowerCase().includes('eko') || 
             data.selectedService.toLowerCase().includes('eco'))) ||
             data.shippingMethod === 'eco';
          // ECO shipping: 45 cents, Standard shipping: $4.50 (450 cents)
          const ddpProcessingFee = data.shippingTerms === 'ddp' ? 
            (isEcoShipping ? 45 : 450) : 0;
